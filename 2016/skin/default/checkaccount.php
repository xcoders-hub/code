...
if (isset($_POST["check"])) {
	...
	if(count($acc["accounts"])>0){
		...
		foreach($acc["accounts"] as $account){
			...
			echo '<tr class="flistmouseoff" align="center"><td><B>'.$account.'</B></td><td>'.ucwords("$type").'</td><td id="unknown-'.$check.'">'.$msg[0].'</td><td id="unknown-'.$check.'">'.$msg[1].'</td></tr>';
			$i++;
		}
		echo "</table>";
	}
	...
	foreach($obj->acc as $host => $acc){
		if(count($acc["accounts"])>0){
			...
			foreach($acc["accounts"] as $account){
				...
				if(!$obj->isadmin()) $account = substr($account, 0, 5)."****";
				echo '<tr class="flistmouseoff" align="center"><td><B>'.$account.'</B></td><td>'.ucwords("$type").'</td><td id="unknown-'.$host.'">'.$obj->lang['unknown'].'</td><td id="unknown-'.$host.'">'.$obj->lang['unknown'].'</td></tr>';
			}
			echo "</table>";
