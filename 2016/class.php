...

{
	...
	function set_config(){
		...
		$this->urlcbox = $this->config['cbox url'];
	}
	function isadmin(){
...
{ 
	...
	function get($url)
	{
		...
		$url = trim($url);
		$host_check = strtolower(parse_url($url, PHP_URL_HOST));
		$images = str_replace('www.', '', $host_check);
		$icon = '[img]https://www.google.com/s2/favicons?domain='.$images.'[/img]';
		if (empty($url)) return;
		$Original = $url;
		$link = '';
		$user = '';
		...
		if(isset($_POST['autosearchuser']) && $_POST['autosearchuser'] == 1) {
			$name = ""; $vipmember = "";
			
			$domain = parse_url($Original, PHP_URL_HOST);
						
			$content = file_get_contents("http://www4.cbox.ws/box/index.php?boxid=4240872&boxtag=soigia&sec=main");
			...
			for($i=2; $i<count($matches); $i++) {
				...
				if (preg_match_all('/<a class="autoLink" href="(.*)" target="_blank">/i', $chat, $temp, PREG_PATTERN_ORDER)) {
					foreach ($temp[1] as $linkcheck) { 
						if (stristr($linkcheck, $Original)) {
							preg_match('/<b class="(.*?)">(.*?)<\/b>/', $mess, $mem);
							$name = $mem[2]; //Get User Name
							$vipmember = false;
							$data = $this->curl("http://vnz-leech.com/vip/group.php","","",0);
							preg_match('/var\s*?gvip1\s*?=(.*?);/i', $data, $list_vip);
							if (stristr($list_vip[1], '"'.$name.'"') || stristr($list_vip[1], "'".$name."'")) $vipmember = true;
							break;
						}
						//else $name = 'Where Are You :tim';
					}
					if ($vipmember!="") break;
				}
			}
			if ($name!="") {
				if ($vipmember == true) $this->bbcode = "[b]  ". $icon ." [vang]@".urlencode(htmlspecialchars($name))."[/mau][/b] [img]http://i.imgur.com/2S8cv5w.gif[/img]";
				else $this->bbcode = "[b]  ". $icon . " [color=blue]@".urlencode(htmlspecialchars($name))."[/color][/b][img]http://i.imgur.com/2S8cv5w.gif[/img]";
			}
		}
		$array_mau = array("[cam]","[tim]","[hong]","[den]","[vang]");
		$mau = $array_mau[rand(0, count($array_mau)-1)];
		if(isset($_POST['autopcbox']) && $_POST['autopcbox'] == 1) {
			$key_get = trim($_POST['pass']);
			$name = $_POST['nick'];
			$this->bbcode .= "[center][b][url=".$lik."] $mau ☆•°ღ•VNZ♥TEAMღ°•☆ [/mau] [color=".$this->colorfn."]| ".$filename."[/color] [color=".$this->colorfs."](".$msize.")[/color][/url][/b] [/center]";
			$data = $this->curl("http://www4.cbox.ws/box/index.php?boxid=4240872&boxtag=soigia&sec=submit", "", "nme=".$name."&key=".$key_get."&pst=".$this->bbcode);
		}
		return $dlhtml;
	}
