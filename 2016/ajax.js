...
function ajaxget(id,url,type){
	...
	captcha = 'none';
	if (type =='reget'){
		$("#link"+id).html("<font color=#FFFF99><b>"+url+"</b></font> <img src='images/loading_black.gif' />"); 
	}
	else if (type !=='get'){
		$("#link"+id).html("reload captcha... <img src='images/loading_black.gif' />"); 
		captcha = 'reload';
	}
	if(i > 3 && type == 'get') time = (Math.round(i/5)*3000);
	else time = 0;
	setTimeout(function(){
		$.ajax({
			type: "POST",
...
function checkacc(type){
	$("table[id='table-"+type+"'] td[id^='unknown']").html('<img src="images/loading_black.gif" />');
	$.ajax({
		type: "POST",
...
$("#autopost").live('click',function(){
	if($(this).is(":checked")) {
		alert(unescape("L\u01b0u \u00fd: Ch\u1ee9c n\u0103ng n\u00e0y s\u1ebd kh\u00f4ng l\u01b0u l\u1ea1i nick v\u00e0 pass!"));
		$("#formauto").show(800);
	}
	else {
		$("#formauto").hide(800);
