...
class dl_fshare_vn extends Download {
	...
	public function CheckAcc($cookie){
		...
		if(stristr($data, 'VIP') && stristr($data, '<dt>Hạn dùng</dt>')) return array(true, "Until ".$this->lib->cut_str($this->lib->cut_str($data, '<dt>Hạn dùng</dt>','<dt>Fxu hiện có</dt>'), '<dd>','</dd>'));
		elseif(stristr($data, 'PROMO') && stristr($data, '<dt>Hạn dùng</dt>')) return array(true, "Until ".$this->lib->cut_str($this->lib->cut_str($data, '<dt>Hạn dùng</dt>','<dt>Fxu hiện có</dt>'), '<dd>','</dd>'));
		elseif(stristr($data, 'PROMO PLUS') && stristr($data, '<dt>Hạn dùng</dt>')) return array(true, "Until ".$this->lib->cut_str($this->lib->cut_str($data, '<dt>Hạn dùng</dt>','<dt>Fxu hiện có</dt>'), '<dd>','</dd>'));
		elseif(stristr($data, 'BUNDLE') && stristr($data, '<dt>Hạn dùng</dt>')) return array(true, "Until ".$this->lib->cut_str($this->lib->cut_str($data, '<dt>Hạn dùng</dt>','<dt>Fxu hiện có</dt>'), '<dd>','</dd>'));
    ...
    public function Login($user, $pass){
		...
		$user = urlencode($user);
		$csrf = $csrf[1];
		$data = $this->lib->curl("https://www.fshare.vn/login", $cookie, "fs_csrf={$csrf}&LoginForm[email]={$user}&LoginForm[password]={$pass}&LoginForm[checkloginpopup]=0&LoginForm[rememberMe]=0&yt0=Đăng+Nhập");
		$cookie = $this->lib->GetCookies($data);
		return $cookie;
    }
    ...
    public function Leech($url) {
		...
		elseif(!$this->isredirect($data)) {
			...
			while (true) {
				...
				if(isset($page['msg'])) {
					if (stristr($page['msg'],"vui lòng thử lại")) continue;
...
* Fix check acc by Sky™ [06.05.2016]
*/
?>