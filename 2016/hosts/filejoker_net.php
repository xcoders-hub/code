...
class dl_filejoker_net extends Download {
    ...
    public function CheckAcc($cookie){
        ...
        if(stristr($data, 'Premium account expires') && stristr($dt, ">Extend Premium<")) return array(true, "Until ".$this->lib->cut_str($dt, "Premium account expires: ", '				<a h').'<br>Traffic Available: '.$this->lib->cut_str($this->lib->cut_str($data, "td>Traffic Available:</td>", "</tr>"), 'td>', '</td'));
    ...
    public function Leech($url) {
		...
		if(stristr($data,'File Not Found')) $this->error("dead", true, false, 2);
			...
			if (strstr($data, "You have reached your download limit:")) $this->error("LimitAcc", true, false, 2);
