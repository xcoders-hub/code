...
class dl_up_4share_vn extends Download {
    ...
    public function CheckAcc($cookie) {
        ...
        elseif(stristr($data, '>Ngày hết hạn: <')) return array(true, 'Until: ' .$this->lib->cut_str($data, 'Ngày hết hạn: <b>', '</b> (còn'). '<br/> Traffic avaiable:' .$this->lib->cut_str($data, 'Bạn đã download từ 4Share hôm nay : <strong>', '</strong> [Tất cả: <strong>').'  '.$this->lib->cut_str($data, 'Bạn còn được download <strong>', '</strong> GB    <br/>'));
